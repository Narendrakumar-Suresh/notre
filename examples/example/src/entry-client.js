...
const routes = Object.fromEntries(Object.entries(allRoutes)
    .filter(([path]) => !path.includes("_layout"))
    .map(([path, loader]) => [toPath(path), loader]));
const layouts = Object.fromEntries(Object.entries(allRoutes)
    .filter(([path]) => path.includes("_layout"))
    .map(([path, loader]) => [toPath(path), loader]));
...
async function main() {
    ...
    if (route) {
        ...
        for (let i = layoutLoaders.length - 1; i >= 0; i--) {
            ...
            CurrentComponent = (componentProps) => (_jsx(LayoutComponent, { ...componentProps, children: _jsx(WrapperComponent, { ...componentProps }) }));
        }
